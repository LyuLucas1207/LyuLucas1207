...
export function LanguageTransitionOverlay({ request, onMidpoint, onComplete }: Props) {
  ...
  const languageRequest = request?.type === 'language' ? request : null

  const shellBackground = useMemo(() => {
    if (!languageRequest) return undefined
  ...
  }, [languageRequest?.id, currentTheme])

  useGSAP(
    () => {
      const node = ref.current
      if (!node || !languageRequest) return
    ...
    { scope: ref, dependencies: [languageRequest?.id, reducedMotion, onMidpoint, onComplete] },
  )

  if (!languageRequest) return null
    ...
    <div ref={ref} className={styles.root}>
      <div className={styles.shell} style={shellBackground ? { background: shellBackground } : undefined} data-overlay-shell>
        ...
        <div className={styles.graphic}>
          <LanguageMoodGraphics language={languageRequest.language} className={styles.svg} />
        </div>
      </div>
    </div>
  )
